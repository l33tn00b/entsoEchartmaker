...
openssl req -x509 -newkey rsa:2048 -keyout /etc/ssl/private/nginx-private-key.key -out /etc/ssl/private/nginx-certificate.crt -sha256 -days 3650 -nodes -subj  "/C=CA/ST=QC/O=TibberTimeframe Inc/CN=TibberTimeframeServer.com"
...
cp /var/tmp/default-ssl /etc/nginx/sites-available
# clean up
rm /var/tmp/default-ssl
